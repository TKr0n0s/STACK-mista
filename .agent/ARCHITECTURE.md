...
```plaintext
/
├── CLAUDE.md                    # Main rules (auto-loaded by Claude Code)
...
└── .claude/
    ├── settings.local.json      # Project settings (generated by setup.sh)
    └── plugins/antigravity/     # Claude Code Plugin
        ├── plugin.json          # Plugin manifest
        ├── skills/              # 18 Workflows as slash commands
        └── agents/              # 20 Agents as subagents
```
